...
window.showStudentDetailModal = async function(userId) {
    ...
    const val = student.points_balance * (appState.settings.point_aed_value || 0.05);
...
window.loadAdminMetrics = async function() {
    try {
        ...
        if (redeemedEl) redeemedEl.textContent = `AED ${formatNumber(data.points_redeemed * (appState.settings.point_aed_value || 0.05))}`;
            ...
            <tr>
                ...
                <td>
                    <div style="display:flex; gap:0.5rem;">
                        <button class="btn" style="background:rgba(223, 177, 91, 0.1); color:#dfb15b; border:1px solid rgba(223, 177, 91, 0.2); padding:0.4rem 0.8rem; font-size:0.8rem;" onclick="openEditPartnerModal('${p.id}')"><i class="fa-solid fa-pen"></i> Edit</button>
...
document.addEventListener('DOMContentLoaded', () => {
    // Attach FAQ Answer Submit Listener
...
// Load Public FAQs on the landing page
async function loadPublicFAQs() {
    const container = document.getElementById('public-faq-container');
    if (!container) return;
