...
function drawPageShell(pageNum, totalPages) {
    ...
    doc.fillColor(colorTextMuted)
       .font('Helvetica')
       .fontSize(8)
       .text(`BIA Enterprise Loyalty Engine Proposal | Page ${pageNum} of ${totalPages}`, 45, 810, { align: 'right' });
}
...
// ============================================================================
doc.rect(45, 35, 505, 170).fill(colorPrimary);
doc.fillColor('#ffffff')
   .font('Helvetica-Bold')
   .fontSize(20)
   .text('ENTERPRISE LOYALTY ENGINE', 70, 65)
   .fontSize(20)
   .text('SPECIFICATION PROPOSAL', 70, 90)
   .fontSize(11)
   .font('Helvetica')
   .text('A Multi-Brand Referral, Tier-Progression, & Gamification Ecosystem', 70, 122)
   .fontSize(9)
   .fillColor('#cbd5e1')
   .text('CLIENT: BRADFORD INTERNATIONAL ALLIANCE (SHARJAH OFFICE)', 70, 155)
   .text('DOCUMENT VERSION: 3.1 (UX & ANIMATIONS ADDED)', 70, 168);
...
const tocItems = [
    ...
    { page: 3, title: '2. Points Accrual Basis & Event Triggers' },
    { page: 4, title: '3. Relational Database Schema & Architecture Blueprint' },
    { page: 5, title: '4. API Endpoint Specifications & Ledger Webhooks' },
    { page: 6, title: '5. Shukran-Style Tier Management & Multipliers' },
    { page: 7, title: '6. Points Expiration Algorithms & Expiry Mathematics' },
    { page: 8, title: '7. User Experience (UX), Tech Stack & Animations (GSAP)' },
    { page: 9, title: '8. WhatsApp Business API & LMS Integrations' },
    { page: 10, title: '9. Security Protocols, Anti-Fraud Middleware, & Compliance' },
    { page: 11, title: '10. Implementation Timelines (Intern + AI) & QA Test Suites' }
];
...
// ============================================================================
// PAGE 3: POINTS ACCRUAL BASIS & EVENT TRIGGERS
// ============================================================================
doc.addPage();
drawSectionTitle('2. POINTS ACCRUAL BASIS & EVENT TRIGGERS');

doc.fillColor(colorTextDark)
   .font('Helvetica')
   .fontSize(9.5)
   .lineGap(3.5)
   .text('To move beyond a simple referral code and replicate a true enterprise loyalty program like Shukran, students must be rewarded across multiple activities. The engine calculates and allocates points based on four primary pillars:', { align: 'justify' })
   .moveDown(0.8);

doc.font('Helvetica-Bold').fontSize(10).text('A. Referral & Enrollment Milestones')
   .font('Helvetica').fontSize(9).moveDown(0.2)
   .text('Points are awarded when a student refers a friend who registers. Crucially, points are only credited after the referred referee completes enrollment and verifies their first installment payment. This protects BIA from shell signups.')
   .moveDown(0.6);

doc.font('Helvetica-Bold').fontSize(10).text('B. Academic Milestones & LMS Activity (SkillShare)')
   .font('Helvetica').fontSize(9).moveDown(0.2)
   .text('Students earn rewards for active academic engagement. Webhooks from the BIA LMS (e.g. SkillShare) automatically trigger credits when a student finishes an extra-curricular course (100–200 base points) or passes a certification module.')
   .moveDown(0.6);

doc.font('Helvetica-Bold').fontSize(10).text('C. Tuition Fees & Spend-Based Points')
   .font('Helvetica').fontSize(9).moveDown(0.2)
   .text('To reward consistent payment, BIA integrates its ERP/billing system with the loyalty subdomain. Students earn points on a spend-ratio (e.g., 1 point per 50 AED paid towards tuition). This functions as a cash-back reward program to help fund future semesters.')
   .moveDown(0.6);

doc.font('Helvetica-Bold').fontSize(10).text('D. Seminar & Networking Event Attendance')
   .font('Helvetica').fontSize(9).moveDown(0.2)
   .text('Building community is vital for professional networking in Sharjah. Students who attend BIA-hosted physical seminars, guest lectures, or alumni networking galas can scan a portal check-in QR code to instantly claim 50–150 engagement points.');

// ============================================================================
// PAGE 4: DATABASE SCHEMA & ARCHITECTURE BLUEPRINT
// ============================================================================
doc.addPage();
drawSectionTitle('3. RELATIONAL DATABASE SCHEMA & ARCHITECTURE BLUEPRINT');
...
doc.font('Helvetica-Bold').fontSize(10).text('Table 1: users (Tracks balances & tiers)')
   ...
   .text('• user_id (INT, PK, Auto Increment) - Unique student identifier.')
...
doc.font('Helvetica-Bold').fontSize(10).text('Table 3: points_ledger (Double-entry transaction audit log)')
   ...
   .font('Helvetica').fontSize(8.5).moveDown(0.2)
   .text('• INDEX(users.referral_code): Speed up referral link lookups during signup.')
   .text('• INDEX(points_ledger.expires_at, points_ledger.points_remaining): Optimizes nightly cron checks.');

// ============================================================================
// PAGE 5: API ENDPOINT SPECIFICATIONS
// ============================================================================
doc.addPage();
drawSectionTitle('4. API ENDPOINT SPECIFICATIONS & LEDGER WEBHOOKS');
...
// ============================================================================
// PAGE 6: SHUKRAN-STYLE TIERS & MULTIPLIERS
// ============================================================================
doc.addPage();
drawSectionTitle('5. SHUKRAN-STYLE TIER MANAGEMENT & MULTIPLIERS');
...
// ============================================================================
// PAGE 7: POINTS EXPIRATION ALGORITHMS
// ============================================================================
doc.addPage();
drawSectionTitle('6. POINTS EXPIRATION ALGORITHMS & MATHEMATICS');
...
// ============================================================================
// PAGE 8: USER EXPERIENCE & VISUAL DESIGN
// ============================================================================
doc.addPage();
drawSectionTitle('7. USER EXPERIENCE (UX), TECH STACK & ANIMATIONS (GSAP)');

doc.fillColor(colorTextDark)
   .font('Helvetica')
   .fontSize(9.5)
   .lineGap(3.5)
   .text('To achieve an app-like premium feel, the student portal is designed using glassmorphic minimalism and powered by the GreenSock Animation Platform (GSAP). Below is a simple explanation of the design language for non-technical stakeholders:', { align: 'justify' })
   .moveDown(0.8);

doc.font('Helvetica-Bold').fontSize(10.5).text('A. Why GSAP? (Sleek Visual Performance)')
   .font('Helvetica').fontSize(9).moveDown(0.2)
   .text('Unlike heavy video scripts or complex CSS animations that can slow down mobile phones, GSAP is an ultra-fast, professional-grade animation library. It ensures all interface transitions look butter-smooth across all smartphones, tablets, and computers, boosting student satisfaction.')
   .moveDown(0.8);

doc.font('Helvetica-Bold').fontSize(10.5).text('B. Premium Micro-Animations Catalogue')
   .font('Helvetica').fontSize(9).moveDown(0.2)
   .text('• Dynamic Points Rollup: When logging in, points count up dynamically from 0 (like a scoreboard rollup). This creates a sense of reward and accomplishment.')
   .text('• Physics-based Progress Bar: The tier progress bar slides and bounces elastically on load.')
   .text('• Interactive 3D Tilt: The loyalty wallet card tilts and catches light reflections based on the user\'s mouse cursor movements, providing a tactile, premium feeling.')
   .text('• Glowing Milestone Alerts: When a reward unlocks, it pulses with a subtle emerald glow.')
   .moveDown(0.8);

doc.font('Helvetica-Bold').fontSize(10.5).text('C. Simplified Technology Stack')
   .font('Helvetica').fontSize(9).moveDown(0.2)
   .text('• Backend Core: Laravel (PHP). Highly secure, easily updates points in the MySQL database, and shares hosting directly with BIA’s main servers (saving infrastructure costs).')
   .text('• Frontend Interface: Modern HTML/CSS + GSAP JavaScript. Students can check their wallets directly on their mobile browsers without having to install a separate mobile app.');

// ============================================================================
// PAGE 9: WHATSAPP BOT & LMS INTEGRATION
// ============================================================================
doc.addPage();
drawSectionTitle('8. WHATSAPP BUSINESS API & LMS INTEGRATIONS');
...
// ============================================================================
// PAGE 10: SECURITY PROTOCOLS & COMPLIANCE
// ============================================================================
doc.addPage();
drawSectionTitle('9. SECURITY PROTOCOLS, ANTI-FRAUD MIDDLEWARE, & COMPLIANCE');
...
doc.font('Helvetica-Bold').fontSize(10.5).text('A. Self-Referral Prevention Middleware')
   ...
   .text('• Device Fingerprints: Block registration if both referrer and referee shares the same device token.')
...
// ============================================================================
// PAGE 11: TIMELINES & QA TEST SUITE
// ============================================================================
doc.addPage();
drawSectionTitle('10. IMPLEMENTATION TIMELINES & QA TEST SUITE');
...
doc.font('Helvetica-Bold').fontSize(10.5).text('Quality Assurance Testing Checklist')
   .font('Helvetica').fontSize(9).moveDown(0.2)
   .text('The intern must run these testing suites before project handoff:')
   .moveDown(0.4)
...
console.log('11-Page detailed proposal generated!');
