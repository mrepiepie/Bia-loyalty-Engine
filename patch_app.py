...
target_profile = """        populateDashboardPartners();
        await loadStudentVouchers(userId);
    } catch (err) {"""
replace_profile = """        populateDashboardPartners();
        await loadStudentVouchers(userId);
...
target_modal = """    // Open Modal
    const modal = document.getElementById('student-detail-modal');"""
replace_modal = """    // Load student promo history
...
target_promo = """            const response = await fetch(`${API_BASE}/admin/promos`, {
                method: 'POST',
...
replace_promo = """            const response = await fetch(`${API_BASE}/admin/promos`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ code, points_reward, max_uses, occasion })
            });
...
target_promo2 = """            const response = await fetch(`${API_BASE}/admin/promos`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ code, points_reward, max_uses, occasion })
            });
...
if 'async function loadStudentPromoHistory' not in app:
    app += append
