...
const targetRegex = /const payload = \{\s*name: document\.getElementById\('partner-form-name'\)\.value\.trim\(\),\s*title: document\.getElementById\('partner-form-title'\)\.value\.trim\(\),\s*subtitle: document\.getElementById\('partner-form-subtitle'\)\.value\.trim\(\),\s*image: base64Image,\s*logoColor: document\.getElementById\('partner-form-color'\)\.value,\s*rewards: \[\s*\{\s*name: document\.getElementById\('reward-1-name'\)\.value\.trim\(\),\s*cost: parseInt\(document\.getElementById\('reward-1-cost'\)\.value\),\s*cash: document\.getElementById\('reward-1-cash'\)\.value\.trim\(\),\s*icon: "fa-mug-hot"\s*\},\s*\{\s*name: document\.getElementById\('reward-2-name'\)\.value\.trim\(\),\s*cost: parseInt\(document\.getElementById\('reward-2-cost'\)\.value\),\s*cash: document\.getElementById\('reward-2-cash'\)\.value\.trim\(\),\s*icon: "fa-gift"\s*\},\s*\{\s*name: document\.getElementById\('reward-3-name'\)\.value\.trim\(\),\s*cost: parseInt\(document\.getElementById\('reward-3-cost'\)\.value\),\s*cash: document\.getElementById\('reward-3-cash'\)\.value\.trim\(\),\s*icon: "fa-car-side"\s*\}\s*\]\s*\};/g;
...
const originalLen = app.length;
app = app.replace(targetRegex, replacement);

if (app.length !== originalLen) {
